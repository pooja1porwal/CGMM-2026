...
from .input_processing import DecodedRaster
...
def _render_raster_tensor(
    image: Image.Image, image_size: tuple[int, int], letterbox: bool, normalize: bool
) -> tuple[torch.Tensor, tuple[int, int, int, int]]:
    """Convert a decoded RGB image using the same tensor contract as SVG input."""
    H, W = image_size
...
class PolygonExtractor:
    ...
    @torch.no_grad()
    def predict_image(self, decoded: DecodedRaster) -> tuple[np.ndarray, tuple[int, int, int, int]]:
        """Run the unchanged model on a decoded PNG/JPEG/PDF page."""
        image_t, rect = _render_raster_tensor(
            decoded.image, self.image_size, self.letterbox, self.normalize
        )
        logits = self.model(image_t.unsqueeze(0).to(self.device))
        mask = logits.argmax(dim=1).squeeze(0).to("cpu", torch.uint8).numpy()
        return mask, rect
    ...
    def extract_image(self, decoded: DecodedRaster) -> ExtractionResult:
        """Extract polygons from a decoded raster/PDF page."""
        mask, (left, top, inner_w, inner_h) = self.predict_image(decoded)
        if (inner_h, inner_w) != mask.shape:
            cleaned = np.full_like(mask, FLOOR_ID)
            cleaned[top:top + inner_h, left:left + inner_w] = mask[top:top + inner_h, left:left + inner_w]
            mask = cleaned
