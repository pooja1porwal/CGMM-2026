...
from buildingcv.input_processing import decode_raster_upload, encode_preview, extension_for_upload
...
def _attach_raster_input_image(image, result: dict) -> dict:
    """Attach a preview in the same content coordinate space as the polygons."""
    _, _, inner_w, inner_h = result["content_rect"]
    result["input_image_b64"] = encode_preview(image, (inner_w, inner_h))
    return result
...
async def extract(svg: UploadFile = File(...)) -> dict:
    """Run the model on an uploaded SVG, raster image, or single-page PDF.

    The multipart field remains named ``svg`` for backwards compatibility with
    existing clients. SVG keeps its original path; raster and PDF inputs are
    decoded by the input-processing layer and use the same model tensor and
    ...
    try:
        extension = extension_for_upload(svg.filename)
    ...
    if extension != ".svg":
        try:
            decoded = decode_raster_upload(raw, extension)
            result = extractor.extract_image(decoded)
        except Exception as e:
            raise HTTPException(status_code=422, detail=f"failed to decode or extract: {e}") from e
        return _attach_raster_input_image(decoded.image, result)

    # Preserve the existing SVG path exactly: PolygonExtractor still parses
    # the temporary file and applies the CubiCasa structural SVG filtering.
